...
public class EventBus implements Runnable{
	...
	public EventBus(TaskScheduler taskScheduler, Embedded core){
	...
	public void publish(Object event){
		try {
			...
			throw new ChaoException(String.format("error publishing %s to EventBus", event), e);
		}
	}
	...
	private List<HandlerDef> getEventType(Object handler) {
		...
		for(Method method : methods){
			if(method.isAnnotationPresent(EventListener.class)){
				...
				if(clazz.length == 2 && Embedded.class ==clazz[1]){
