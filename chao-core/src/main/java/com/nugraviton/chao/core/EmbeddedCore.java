...
import com.nugraviton.chao.core.event.JobSDeployedEvent;
import com.nugraviton.chao.core.event.CoreStartedEvent;
import com.nugraviton.chao.core.event.DeploymentEvent;
...
public class EmbeddedCore implements Core, TaskExecutable{
	
	private static Logger log = LoggerFactory.getLogger(EmbeddedCore.class);
	...
	public EmbeddedCore(){
		try {
			...
			throw new CoreException("error creating core.", e);
		}
	}
	...
	public void start() {
        ...
        try {
			...
			if(!started.get()){
				...
				started.set(true);
				eventBus.publish(new CoreStartedEvent());
