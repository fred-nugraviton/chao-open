...
import com.nugraviton.chao.core.WorkflowBuilder;
...
class DeploymentListener{
	...
	@EventListener
	public void listen(DeploymentEvent event, EmbeddedCore core) throws IOException{
		...
		try{
			...
			Method deployJob = core.getClass().getDeclaredMethod("deployJobs", Map.class);
			deployJob.setAccessible(true);
			deployJob.invoke(core, wfs);
			deployJob.setAccessible(false);
			
		}catch(Throwable e){
			core.publishExceptionEvent(e);
		}
	}
